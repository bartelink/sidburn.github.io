...
I think that looking at this questions and there answers is still a very important step forward in understanding programming and
...
So why should something like that be done? The reason was that we anyway had this concepts, but implicitly. All of these ideas
...
That is the very reason of why you can get a "StackOverflow" if your language don't support tail-recursion.
...
All of those languages usually provide some kind of control structures, in the form of looping (while, for, foreach). Subroutines
in the form of procedures, functions, (static) methods or however your language names them. Selection in the form of 
(if, pattern matching, switch) statements and so on.
...
Instead of typical while or foreach loops C# for example provides LINQ, Java 8 introduced Stream as a more declarative
alternative.
...
the introduction of (if, while, for, functions, blocks) it was the general concept of eliminating something powerful and provide
more declarative and specific control structures instead. 
...
For example you can can loop with foreach through an array to find the smallest element, the biggest element. You can apply a function
to every element. You can filter an array. You can loop through two array, somehow combine them, do some calculations with every element
and so on. Or in other words. We can do a lot with such a simple constructs. And that is why we view them as "harmful".
...
and so one just to name a view. All of those constructs are still basically just "looping" constructs. But they are specializes looping
